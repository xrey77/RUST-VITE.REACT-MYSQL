...
pub async fn get_productsearch(
    ...
    let totalrecords_count: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM products WHERE descriptions LIKE ?")
        .bind(&search_pattern)
    ...
    let users_result: Vec<Products> = sqlx::query_as(r#"SELECT id,category,descriptions,qty,unit,costprice,sellprice,saleprice,productpicture,alertstocks,criticalstocks FROM products  WHERE descriptions LIKE ? LIMIT ?, ?"#)
    .bind(&search_pattern)
    .bind(&offset)
    .bind(&per_page)
